...
async def main():
    ...
    service = MemoryService(
        ...
        memorize_config=MemorizeConfig(
                                ...
                                memory_categories=[
                                    CategoryConfig(name="Agents", description="Personal details"),
                                    CategoryConfig(name="RAG", description="Likes and dislikes"),
                                    CategoryConfig(name="LLM Training", description="Past events and experiences"),
                                    CategoryConfig(name="LLM Inference", description="Past events and experiences")
                                ]
                            ) 
    ...
    items = await service.list_memory_items(where={"user_id": "user4568", "workspace_id": "workspace-alphahbj"})
    print(f"Found {len(items.get('items', []))} memory items")
    
    # # List categories
    print("\n🏷️  Listing categories...")
    categories = await service.list_memory_categories(where={"user_id": "user4568", "workspace_id": "workspace-alphahbj"})
